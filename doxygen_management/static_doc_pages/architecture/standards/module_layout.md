...
## Reference layout (maximal envelope)
    ...
    │   └── internal/
    │       ├── module_ctx.h
    │       └── module_cfg.h    (optional, bootstrap-only configuration)
    ├── src/
    │   ├── module.c
    │   └── internal/
...
Note  <here> (point 2)
`<module>_t` is the only runtime object visible to normal consumers.  
Consumers must never depend on `_ctx_t`, `_deps_t`, or wiring helpers.
...
- module_cfg.h — optional bootstrap-only configuration (`*_cfg_t`).

Key principles:

- ctx and internal cfg are wiring-time objects, not runtime objects;
...
- consumes module_ctx.h and/or internal module_cfg.h only at construction time;
...
### tests/include/ — Test API (optional)
...
### tests/support/ — Test-only support code (optional)

The tests/support/ directory contains test-only helper libraries for this
module. This code is not part of production builds.

Typical use cases:

- `<module>_fake_provider` (facade for retrieving/configuring reusable fakes)
- module-scoped helpers for wiring/injection in other modules' tests

Note  
Reusable fake implementations themselves live under `lexleo/tests/test_support/`
(e.g., `fake_<module>`). This module-level `tests/support/` layer exists to
provide a local facade and optional wiring glue.
...
Additionally, test-related targets follow:

- `<module>_test_headers` exports `tests/include/`
- `<module>_test_usage_requirements` bundles module-specific test requirements
- `<module>_wiring_headers` is reserved for wiring/injection consumers

These rules ensure architectural boundaries are enforced at build time.
...
-------------------|------------------------------------
 ...
 include/internal/ | Wiring and lifecycle orchestration 
 src/              | Runtime implementation             
 src/internal/     | Private runtime details            
 tests/            | Unit and integration tests         
 tests/support/    | Module-local test-only support code

---

This layout is explicitly aligned with the Type standards document.
