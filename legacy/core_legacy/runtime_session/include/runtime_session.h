// src/core/runtime_session/include/runtime_session.h
...
//   appends, stream_legacy replacement, etc.) is automatically reflected in the
//   scanner via the YY_INPUT hook set by input_provider_legacy_bind_to_scanner().
//
// Ownership:
...
bool runtime_session_bind_output_stream(
	runtime_session *session,
	struct stream_legacy *out); // borrowed

struct stream_legacy *runtime_session_get_output_stream_legacy(
