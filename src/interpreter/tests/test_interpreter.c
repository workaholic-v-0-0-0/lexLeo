...
static runtime_env_value A_NUMBER_RUNTIME_ENV_VALUE = {.type = RUNTIME_VALUE_NUMBER, .refcount = 2, .as.i = A_INT};
...
static runtime_env_value DUMMY_RUNTIME_ENV_VALUE = {.type = RUNTIME_VALUE_NUMBER, .refcount = 2, .as.i = A_INT};
...
typedef struct {
    ...
    bool runtime_env_get_returned_a_number_runtime_value;
...
static int parametric_setup(void **state) {
    ...
    if (p->runtime_env_get_will_be_called_a_first_time && !p->runtime_env_get_will_fail) {
        ...
        else if (p->runtime_env_get_returned_a_number_runtime_value)
            p->ctx->spy.fake_value_bound_to_dummy_symbol_p =
                runtime_env_make_number(A_INT);
        else
            p->ctx->spy.fake_value_bound_to_dummy_symbol_p =
                runtime_env_make_number(A_INT);
    }
...
static int parametric_teardown(void **state) {
    ...
    runtime_env_value_release(p->ctx->spy.fake_value_bound_to_dummy_symbol_p);//debug
...
static const test_interpreter_case NEGATION_NODE_WITH_SYMBOL_AND_BOUND_TO_A_NUMBER_AND_OOM = {
    ...
    .runtime_env_get_will_be_called_a_first_time = true,
    .runtime_env_get_will_fail = false,
    .runtime_env_get_returned_a_number_runtime_value = true,
...
static const test_interpreter_case ADDITION_NODE_LHS_NUMBER_RHS_SYMBOL_BOUND_TO_NUMBER = {
    .name = "eval_succees_when_addition_node_with_lhs_number_rhs_symbol_to_bound_number",
...
static const test_interpreter_case SUBTRACTION_NODE_LHS_NUMBER_RHS_SYMBOL_BOUND_TO_NUMBER = {
    .name = "eval_succees_when_subtraction_node_with_lhs_number_rhs_symbol_bound_to_number",
