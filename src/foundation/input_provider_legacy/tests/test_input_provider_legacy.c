// src/foundation/input_provider_legacy/tests/test_input_provider_legacy.c
...
static stream_legacy *DUMMY_BORROWED_STREAM = (stream_legacy *) 0xBAADF00D;
...
typedef struct test_input_provider_legacy_deep_snapshot {
	input_provider_legacy_mode mode;
	stream_legacy *borrowed_stream;
	stream_legacy *chunks_stream;
	char *buf_address;
	char *buf_content_p;
	size_t cap;
	size_t len;
...
} test_input_provider_legacy_deep_snapshot;

static test_input_provider_legacy_deep_snapshot input_provider_legacy_deep_snapshot(const input_provider *p) {
	assert_non_null(p);
	test_input_provider_legacy_deep_snapshot ret = {
		.mode = p->mode,
		.borrowed_stream = p->borrowed_stream,
		.chunks_stream = p->chunks_stream,
		.buf_address = (p->chunks_stream)
			               ? ((dynamic_buffer_stream_legacy_ctx *) stream_legacy_get_ctx(p->chunks_stream))->buf
			               : NULL,
		.cap = (p->chunks_stream) ? ((dynamic_buffer_stream_legacy_ctx *) stream_legacy_get_ctx(p->chunks_stream))->cap : 0,
		.len = (p->chunks_stream) ? ((dynamic_buffer_stream_legacy_ctx *) stream_legacy_get_ctx(p->chunks_stream))->len : 0,
		.read_pos = (p->chunks_stream) ? ((dynamic_buffer_stream_legacy_ctx *) stream_legacy_get_ctx(p->chunks_stream))->read_pos : 0,
		.autoclose = (p->chunks_stream)
			             ? ((dynamic_buffer_stream_legacy_ctx *) stream_legacy_get_ctx(p->chunks_stream))->autoclose
			             : false,
		.lexer_scanner = p->lexer_scanner
	};
	if (ret.chunks_stream) {
		ret.buf_content_p = INPUT_PROVIDER_MALLOC(
			sizeof(char) * ((dynamic_buffer_stream_legacy_ctx *) stream_legacy_get_ctx(p->chunks_stream))->cap);
		assert_non_null(ret.buf_content_p);
		memcpy(ret.buf_content_p, ((dynamic_buffer_stream_legacy_ctx *) stream_legacy_get_ctx(p->chunks_stream))->buf,
		       ((dynamic_buffer_stream_legacy_ctx *) stream_legacy_get_ctx(p->chunks_stream))->cap);
	} else {
...
static void input_provider_legacy_deep_snapshot_destroy(test_input_provider_legacy_deep_snapshot *s) {
	if (!s) return;
	if (s->buf_content_p) {
...
static void assert_same_input_provider_as_snapshot(const input_provider *p,
                                                   const test_input_provider_legacy_deep_snapshot *s) {
	assert_non_null(p);
	assert_non_null(s);
	...
	if (p->chunks_stream) {
		dynamic_buffer_stream_legacy_ctx *dbs = ((dynamic_buffer_stream_legacy_ctx *) stream_legacy_get_ctx(p->chunks_stream));
...
//  - input_provider_legacy_destroy(NULL) is safe (no crash)
static void create_returns_null_when_oom(void **state) {
	(void) state;
	fake_memory_fail_on_all_call();

	input_provider *ret = input_provider_legacy_create();
	...
	input_provider_legacy_destroy(ret);
}
...
//  - No leaks, no invalid or double frees
//  - input_provider_legacy_destroy(ret) properly frees all owned resources
...
//-----------------------------------------------------------------------------
// TESTS
// bool input_provider_legacy_bind_to_scanner(
//     input_provider *p,
//     yyscan_t scanner );
//-----------------------------------------------------------------------------
...
static void bind_to_scanner_returns_false_when_p_null(void **state) {
	(void) state;
	bool ret = input_provider_legacy_bind_to_scanner(NULL, DUMMY_SCANNER);
	assert_false(ret);
}
...
static void bind_to_scanner_returns_false_when_scanner_null(void **state) {
	(void) state;
	bool ret = input_provider_legacy_bind_to_scanner(DUMMY_INPUT_PROVIDER_P, NULL);
	assert_false(ret);
}
...
static void bind_to_scanner_returns_false_when_already_bound(void **state) {
	...
	bool ret = input_provider_legacy_bind_to_scanner(DUMMY_INPUT_PROVIDER_P, DUMMY_SCANNER);
	assert_false(ret);
...
static void bind_to_scanner_succeeds_when_first_bind(void **state) {
	(void) state;
	input_provider *p = input_provider_legacy_create();
	assert_non_null(p);
	assert_null(p->lexer_scanner);
	test_input_provider_legacy_deep_snapshot snapshot = input_provider_legacy_deep_snapshot(p);

	bool ret = input_provider_legacy_bind_to_scanner(p, DUMMY_SCANNER);
	...
	input_provider_legacy_destroy(p);
	input_provider_legacy_deep_snapshot_destroy(&snapshot);
}
...
static void set_mode_chunks_returns_false_when_p_null(void **state) {
	(void) state;
	bool ret = input_provider_legacy_set_mode_chunks(NULL);
	assert_false(ret);
}
...
static void set_mode_chunks_success_when_p_not_null_and_chunks_stream_null(void **state) {
	...
	bool ret = input_provider_legacy_set_mode_chunks(p);
...
static void set_mode_chunks_success_when_p_not_null_and_chunks_stream_not_null(void **state) {
	...
	input_provider *p = input_provider_legacy_create();
	assert_non_null(p);
	assert_true(input_provider_legacy_set_mode_chunks(p));
	assert_int_equal(p->mode, INPUT_PROVIDER_MODE_CHUNKS);
	assert_non_null(p->chunks_stream);
	test_input_provider_legacy_deep_snapshot snapshot = input_provider_legacy_deep_snapshot(p);
...
//-----------------------------------------------------------------------------
// TESTS
// bool input_provider_legacy_set_mode_borrowed_stream(input_provider *p, stream *s);
//-----------------------------------------------------------------------------
...
static void set_mode_borrowed_stream_returns_false_when_p_null(void **state) {
	(void) state;
	bool ret = input_provider_legacy_set_mode_borrowed_stream(NULL, DUMMY_BORROWED_STREAM);
	assert_false(ret);
}
...
static void set_mode_borrowed_stream_returns_false_and_no_side_effect_when_s_null(void **state) {
	...
	bool ret = input_provider_legacy_set_mode_borrowed_stream(p, NULL);
...
static void set_mode_borrowed_stream_success_when_p_not_null_and_s_not_null(void **state) {
	...
	bool ret = input_provider_legacy_set_mode_borrowed_stream(p, DUMMY_BORROWED_STREAM);
...
static void append_returns_false_when_p_null(void **state) {
	(void) state;
	bool ret = input_provider_legacy_append(NULL, DUMMY_STRING, DUMMY_SIZE_T);
	assert_false(ret);
}
...
static void append_returns_false_when_bytes_null(void **state) {
	...
	bool ret = input_provider_legacy_append(p, NULL, DUMMY_SIZE_T);
...
static void append_success_when_p_is_valid_and_no_cap_exceeding(void **state) {
	...
	bool ret = input_provider_legacy_append(p, bytes, len);
...
static void read_returns_0_when_p_null(void **state) {
	(void) state;
	size_t ret = input_provider_legacy_read(NULL, DUMMY_STRING, DUMMY_SIZE_T);
...
static void read_returns_0_when_buf_null(void **state) {
	...
	size_t ret = input_provider_legacy_read(p, NULL, DUMMY_SIZE_T);
...
static void read_returns_0_when_n_0(void **state) {
	...
	size_t ret = input_provider_legacy_read(p, DUMMY_STRING, 0);
...
static void read_success_when_no_cap_exceeding(void **state) {
	...
	assert_true(input_provider_legacy_set_mode_chunks(p));
	assert_non_null(p->chunks_stream);
	assert_true(input_provider_legacy_append(p, "abcdefg", 7));
	dynamic_buffer_stream_legacy_ctx *dbs = (dynamic_buffer_stream_legacy_ctx *) stream_legacy_get_ctx(p->chunks_stream);
	assert_non_null(dbs);
	assert_non_null(dbs->buf);
	dbs->read_pos = 3;
	...
	size_t ret = input_provider_legacy_read(p, buf, 10);
