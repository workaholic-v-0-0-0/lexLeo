...
static void test_stdio_stream_t_lifecycle(void **state) {
	...
	if (tc->scenario == STDIO_STREAM_T_LIFECYCLE_SCENARIO_OK_STDIO_INJECTED_BY_TEST_INFRA) {
		stdio_stream_inject_stdio(stdio_stream, fake_stdio_to_osal_stdio(&fake_stdio));
	}
	...
	switch (tc->scenario) {
		case STDIO_STREAM_T_LIFECYCLE_SCENARIO_OK_STDIO_INJECTED_BY_TEST_INFRA:
			assert_int_equal(g_fake_stdio_ctrl.get_stdin_call_count, 0);
			assert_int_equal(g_fake_stdio_ctrl.get_stdout_call_count, 0);
			assert_int_equal(g_fake_stdio_ctrl.get_stderr_call_count, 0);
			expected_stdio = fake_stdio_to_osal_stdio(&fake_stdio);
			break;
		case STDIO_STREAM_T_LIFECYCLE_SCENARIO_OK_STDIO_FETCHED_BY_DEFAULT_INIT:
			switch (tc->kind) {
				case STREAM_STANDARD_STREAM_KIND_STDIN:
					assert_int_equal(g_fake_stdio_ctrl.get_stdin_call_count, 1);
					expected_stdio = fake_stdio_to_osal_stdio(&g_fake_stdio_ctrl.stdin);
					break;
				case STREAM_STANDARD_STREAM_KIND_STDOUT:
					assert_int_equal(g_fake_stdio_ctrl.get_stdout_call_count, 1);
					expected_stdio = fake_stdio_to_osal_stdio(&g_fake_stdio_ctrl.stdout);
					break;
				case STREAM_STANDARD_STREAM_KIND_STDERR:
					assert_int_equal(g_fake_stdio_ctrl.get_stderr_call_count, 1);
					expected_stdio = fake_stdio_to_osal_stdio(&g_fake_stdio_ctrl.stderr);
					break;
				default: fail();
			}
			break;
