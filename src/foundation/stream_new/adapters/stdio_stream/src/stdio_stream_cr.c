	...
	stream_status_t *st)
{
	...
	size_t ret = s->stdio_ops->read(buf, 1, n, s->stdio);

	if (s->stdio_ops->error(s->stdio)) {
		if (st) *st = STREAM_STATUS_IO_ERROR;
	} else if (s->stdio_ops->eof(s->stdio)) {
	...
	stream_status_t *st)
{
	...
	size_t ret = s->stdio_ops->write(buf, 1, n, s->stdio);
...
static stream_status_t stdio_stream_flush(void *backend)
{
	...
	if (s->stdio_ops->flush(s->stdio) != 0)
		return STREAM_STATUS_IO_ERROR;
