...
// <here> light lib has to be done to include fake stream here
 ...
 *
 * Invalid arguments:
 * - `out`, `cfg`, `env` must not be NULL.
 *
 * Success:
 * - Returns LOGGER_STATUS_OK.
 * - Stores a valid logger handle in `*out`.
 * - The produced logger handle is ready for normal runtime use.
 * - The produced logger handle must be destroyed via `logger_destroy()`.
 *
 * Failure:
 * - Returns:
...
static int setup_logger_default_create_logger(void **state)
{
	...
	const stream_env_t stream_env = { .mem = osal_mem_test_fake_ops() };

	fx->cfg.append_newline = true;

	// DI
	assert_int_equal(
		fake_stream_create(&fx->env.stream, &stream_env),
		STREAM_STATUS_OK
	);
...
static void test_logger_default_create_logger(void **state)
{
	...
	// ARRANGE
	logger_status_t st = (logger_status_t)-1; // poison

	stream_t **out_arg = &fx->out;
	const logger_cfg_t *cfg_arg = &fx->cfg;
