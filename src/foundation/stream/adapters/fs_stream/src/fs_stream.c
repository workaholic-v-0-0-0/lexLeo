	...
	stream_status_t *st )
{
	...
	size_t s = fs_stream->file->read(fs_stream->state.f, buf, n, &osal_st);
	...
	stream_status_t *st )
{
	...
	osal_file_status_t osal_st = OSAL_FILE_ERR;
	size_t s = fs_stream->file->write(fs_stream->state.f, buf, n, &osal_st);
...
static stream_status_t fs_stream_flush(void *backend) {
	...
	osal_file_status_t osal_st = fs_stream->file->flush(fs_stream->state.f);
...
static stream_status_t fs_stream_close(void *backend) {
    ...
    if (fs_stream->state.f && fs_stream->state.autoclose) {
        osal_st = fs_stream->file->close(fs_stream->state.f);
    }
    fs_stream->state.f = NULL;

    LEXLEO_ASSERT(fs_stream->mem && fs_stream->mem->free);
    fs_stream->mem->free(fs_stream);
    ...
    const fs_stream_env_t *env)
{
    ...
    backend->mem = env->file_env.mem;
    backend->file = env->file_ops;
	...
	const fs_stream_env_t *env )
{
    ...
    if (st != STREAM_STATUS_OK) {
        if (backend && backend->mem && backend->mem->free)
			backend->mem->free(backend);
        return st;
    }
    return STREAM_STATUS_OK;
}
	...
	stream_t **out )
{
	...
	return
		fs_stream_create_stream(
			out,
			(const fs_stream_args_t *)args,
			ctor_ud->cfg, // NULL for now ; when evoluting, pput a &
			&ctor_ud->env );
}

void fs_stream_destroy_ud_ctor(
	void *ud,
