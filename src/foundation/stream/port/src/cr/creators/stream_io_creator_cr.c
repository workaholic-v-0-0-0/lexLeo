...
/**
 * @file stream_io_creator_cr.c
 * @ingroup stream_internal_group
 * @brief I/O stream creator lifetime implementation.
 *
 * @details
...
stream_status_t stream_create_io_creator(
	...
	stream_io_creator_t *tmp = mem->calloc(1, sizeof(*tmp));
	if (!tmp) {
...
void stream_destroy_io_creator(
	...
	stream_io_creator_t *tmp = *creator;
	*creator = NULL;
