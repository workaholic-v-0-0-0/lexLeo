 ...
 * - integration-focused exercising of the public factory API
 * - use of a local adapter descriptor helper for upcoming registration tests
 *
 * Local test doubles:
 * - `test_stream_ctor`
 * - `make_test_desc()`
 *
 * See @ref testing_foundation_stream_integration
 * "Stream factory integration test specification" for the editorial
 * behavioral contract.
 */
...
/**
 * @brief Minimal test adapter constructor used by factory integration tests.
 *
 * @details
 * This local helper implements the `stream_ctor_fn_t` signature expected by
 * the stream factory.
 *
 * It does not construct a real `stream_t` instance. Instead, it serves as a
 * lightweight test double for validating generic factory behavior:
 * - `ud` and `args` are accepted but intentionally ignored,
 * - if `out` is NULL, the function returns `STREAM_STATUS_INVALID`,
 * - otherwise, it stores a fixed non-NULL sentinel pointer into `*out`
 *   and returns `STREAM_STATUS_OK`.
 *
 * This allows integration tests to verify that the factory correctly invokes
 * a registered constructor and propagates its result.
 */
static stream_status_t test_stream_ctor(
	const void *ud,
	const void *args,
	stream_t **out)
{
 ...
 * Success:
 * - Returns STREAM_STATUS_OK.
 * - Registers the adapter descriptor in the factory.
 * - The registered adapter can later be resolved by its key through
...
typedef enum {
     ...
     * factory and a valid descriptor whose key is not yet registered
     * EXPECT:
     * - returns `STREAM_STATUS_OK`
     * - registers the descriptor into the factory
     * - the key becomes available for later stream creation
     */
     ...
     * - the factory keeps the previously registered descriptor for that key
     */
     ...
     * - the new descriptor is not registered
     */
...
static int setup_stream_fact_add_adapter(void **state)
{
	...
	fx->desc = make_test_desc("key_test", test_stream_ctor);
	fx->other_desc = make_test_desc("other_key", test_stream_ctor);
...
static void test_stream_fact_add_adapter(void **state)
{
	...
	stream_factory_t *fact_arg = fx->fact;
	...
	// ACT
	ret = stream_status_t stream_factory_add_adapter(fact_arg, desc_arg);
