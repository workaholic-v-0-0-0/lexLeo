 ...
 *
 * See @ref testing_foundation_stream_unit
 * "Stream port unit test specification" for the editorial behavioral contract.
 */
...
static void test_stream_default_ops(void **state) {
	(void)state;
	const stream_ops_t *cut = stream_default_ops();
	assert_non_null(cut);
	assert_non_null(cut->read);
	assert_non_null(cut->write);
	assert_non_null(cut->flush);
}
...
static void test_stream_default_env(void **state) {
	...
	stream_env_t cut = stream_default_env(dummy_p);

	assert_ptr_equal(cut.mem, dummy_p);
}
...
/**
 * @brief Expected state of `out` after the call under test.
 *
 * Notes:
 * - `OUT_EXPECT_UNCHANGED` is verified using a sentinel pointer.
 */
typedef enum {
	OUT_CHECK_NONE,
	OUT_EXPECT_NULL,
...
/**
 * @brief Execute one lifecycle test scenario for `stream_create()` / `stream_destroy()`.
 */
