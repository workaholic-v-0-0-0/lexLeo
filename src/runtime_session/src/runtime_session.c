// src/runtime_session/src/runtime_session.c
...
static const lexer_ops_t RUNTIME_SESSION_LEXER_OPS_DEFAULT = {
...
bool runtime_session_bind_input_provider(
	...
	if (!input_provider_bind_to_scanner(provider, session->scanner)) return false;
	session->in = provider;
	return true;
}
