// src/core/runtime_env/src/runtime_env.c
...
static bool runtime_env_set_local_impl(
        runtime_env *e,
...
static runtime_env_ctx g_runtime_env_ctx = {
    .hashtable_ops = &RUNTIME_ENV_HASHTABLE_OPS_DEFAULT,
...
static runtime_env_value *runtime_env_get_impl(
	...
	return
		g_runtime_env_ctx.hashtable_ops->hashtable_key_is_in_use(e->bindings, key) ?
		g_runtime_env_ctx.hashtable_ops->hashtable_get(e->bindings, key)
		:
...
void runtime_env_set_hashtable_ops(const hashtable_ops_t *overrides) {
    static hashtable_ops_t applied;
    applied = *g_runtime_env_ctx.hashtable_ops;
    ...
    g_runtime_env_ctx.hashtable_ops = &applied;
}

void runtime_env_set_ops(const runtime_env_ops_t *overrides) {
    static runtime_env_ops_t applied;
...
void runtime_env_reset_hashtable_ops(void) {
    g_runtime_env_ctx.hashtable_ops = &RUNTIME_ENV_HASHTABLE_OPS_DEFAULT;
}

void runtime_env_set_create_bindings(create_bindings_fn_t fn) {
    runtime_env_set_hashtable_ops(&(hashtable_ops_t){
        .create_bindings = fn ? fn : RUNTIME_ENV_HASHTABLE_OPS_DEFAULT.create_bindings
    });
}

void runtime_env_set_destroy_bindings(destroy_bindings_fn_t fn) {
    runtime_env_set_hashtable_ops(&(hashtable_ops_t){
        .destroy_bindings = fn ? fn : RUNTIME_ENV_HASHTABLE_OPS_DEFAULT.destroy_bindings
    });
}

void runtime_env_set_hashtable_key_is_in_use(hashtable_key_is_in_use_fn_t fn) {
    runtime_env_set_hashtable_ops(&(hashtable_ops_t){
        .hashtable_key_is_in_use = fn ? fn : RUNTIME_ENV_HASHTABLE_OPS_DEFAULT.hashtable_key_is_in_use
    });
}

void runtime_env_set_hashtable_get(hashtable_get_fn_t fn) {
    runtime_env_set_hashtable_ops(&(hashtable_ops_t){
        .hashtable_get = fn ? fn : RUNTIME_ENV_HASHTABLE_OPS_DEFAULT.hashtable_get
    });
}

void runtime_env_set_hashtable_add(hashtable_add_fn_t fn) {
    runtime_env_set_hashtable_ops(&(hashtable_ops_t){
        .hashtable_add = fn ? fn : RUNTIME_ENV_HASHTABLE_OPS_DEFAULT.hashtable_add
    });
}

void runtime_env_set_hashtable_reset_value(hashtable_reset_value_fn_t fn) {
    runtime_env_set_hashtable_ops(&(hashtable_ops_t){
        .hashtable_reset_value = fn ? fn : RUNTIME_ENV_HASHTABLE_OPS_DEFAULT.hashtable_reset_value
    });
}

void runtime_env_set_hashtable_remove(hashtable_remove_fn_t fn) {
    runtime_env_set_hashtable_ops(&(hashtable_ops_t){
        .hashtable_remove = fn ? fn : RUNTIME_ENV_HASHTABLE_OPS_DEFAULT.hashtable_remove
    });
}

void runtime_env_set_set_local(runtime_env_set_local_fn_t fn) {
    runtime_env_set_ops(&(runtime_env_ops_t){
        .set_local = fn ? fn : RUNTIME_ENV_OPS_DEFAULT.set_local
    });
}
...
destroy_bindings_fn_t runtime_env_get_destroy_bindings(void) {
    return g_runtime_env_ctx.hashtable_ops->destroy_bindings;
}

hashtable_key_is_in_use_fn_t runtime_env_get_hashtable_key_is_in_use(void) {
    return g_runtime_env_ctx.hashtable_ops->hashtable_key_is_in_use;
}

hashtable_get_fn_t runtime_env_get_hashtable_get(void) {
    return g_runtime_env_ctx.hashtable_ops->hashtable_get;
}

hashtable_add_fn_t runtime_env_get_hashtable_add(void) {
    return g_runtime_env_ctx.hashtable_ops->hashtable_add;
}

hashtable_reset_value_fn_t runtime_env_get_hashtable_reset_value(void) {
    return g_runtime_env_ctx.hashtable_ops->hashtable_reset_value;
}

hashtable_remove_fn_t runtime_env_get_hashtable_remove(void) {
    return g_runtime_env_ctx.hashtable_ops->hashtable_remove;
}
