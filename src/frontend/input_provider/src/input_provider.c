...
void input_provider_destroy(input_provider *p) {
	if (!p) return;
	if (p->chunks_stream) stream_close(p->chunks_stream);
...
bool input_provider_set_mode_chunks(input_provider *p) {
	if (!p) return false;
	if (p->chunks_stream) {
		stream_close(p->chunks_stream);
		p->chunks_stream = NULL;
	}
	p->chunks_stream = dynamic_buffer_stream_create();
	if (!p->chunks_stream) {
...
bool input_provider_append(input_provider *p, const char *bytes, size_t len) {
	if (
			...
			|| !p->chunks_stream )
		return false;
	return (len == stream_write(p->chunks_stream, bytes, len));
}
...
bool input_provider_append_line(
		...
		const char *bytes,
		size_t len ) {
	if (!input_provider_append(p, bytes, len)) return false;
	return input_provider_append(p, "\n", 1);
}

bool input_provider_append_string_as_line(
		input_provider *p,
		const char *s) {
	if (!input_provider_append(p, s, strlen(s))) return false;
	return input_provider_append(p, "\n", 1);
}

static stream *input_provider_get_active_stream(const input_provider *p) {
	if (!p) return NULL;
	switch (p->mode) {
