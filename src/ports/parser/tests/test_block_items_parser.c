// src/ports/parser/tests/test_block_items_parser.c
...
#include "block_items_parser.tab.h"
...
static void block_items_parse_create_error_node_for_LHS_when_0_statement_and_create_children_node_var_fails(
    ...
    expect_value(mock_create_error_node_or_sentinel, code, AST_ERROR_CODE_BLOCK_ITEMS_NODE_CREATION_FAILED);
    expect_string(mock_create_error_node_or_sentinel, message, "ast creation for the content of a block node failed");
...
//  - ast_create_children_node_var will fail
// Expected:
//  - epsilon -> block_items triggers a call to ast_create_children_node_var with:
//    - type: AST_TYPE_BLOCK_ITEMS
//    - children_nb: 0
...
//    block_items produced by the epsilon-reduction
...
//  - when reducing block_items : block_items statement
...
static void block_items_parse_cleans_up_and_create_error_node_for_LHS_when_1_statement_and_children_append_take_fails(
    ...
    expect_value(mock_create_children_node_var, type, AST_TYPE_BLOCK_ITEMS);
    expect_value(mock_create_children_node_var, children_nb, 0);
    will_return(mock_create_children_node_var, DUMMY_AST_BLOCK_ITEMS_NODE_P);
    will_return(stub_binding_statement_action, DUMMY_AST_ERROR_OR_BINDING_NODE_P);
    expect_value(mock_children_append_take, parent, DUMMY_AST_BLOCK_ITEMS_NODE_P);
...
// Expected:
//  - epsilon -> block_items
//    - $$ = ast_create_children_node_var(AST_TYPE_BLOCK_ITEMS, 0)
...
int main(void) {
    const struct CMUnitTest block_items_parse_tests[] = {
        cmocka_unit_test_setup_teardown(
            block_items_parse_create_error_node_for_LHS_when_0_statement_and_create_children_node_var_fails,
            block_items_parse_setup, block_items_parse_teardown),
        cmocka_unit_test_setup_teardown(
            block_items_parse_create_block_items_node_for_LHS_when_0_statement_and_create_children_node_var_succeeds,
            block_items_parse_setup, block_items_parse_teardown),
        cmocka_unit_test_setup_teardown(
            block_items_parse_cleans_up_and_create_error_node_for_LHS_when_1_statement_and_create_children_node_var_fails,
            block_items_parse_setup, block_items_parse_teardown),
        cmocka_unit_test_setup_teardown(
            block_items_parse_cleans_up_and_create_error_node_for_LHS_when_1_statement_and_children_append_take_fails,
            block_items_parse_setup, block_items_parse_teardown),
        cmocka_unit_test_setup_teardown(
            block_items_parse_create_block_items_node_for_LHS_when_1_statement_and_children_append_take_succeeds,
            block_items_parse_setup, block_items_parse_teardown),
        cmocka_unit_test_setup_teardown(
            block_items_parse_create_block_items_node_for_LHS_when_3_statement_and_children_append_take_succeeds_3_times,
            block_items_parse_setup, block_items_parse_teardown),
