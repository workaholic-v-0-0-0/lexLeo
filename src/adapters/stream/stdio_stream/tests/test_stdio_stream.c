...
static void from_stdin_ret_null_when_oom(void **state) {
	...
	stream *ret = stdio_stream_from_stdin();
...
static void from_stdin_reads_from_stdin(void **state) {
	...
	stream *s = stdio_stream_from_stdin();
	assert_non_null(s);
	size_t n = stream_read(s, buf, 16);
...
static void to_stdout_ret_null_when_oom(void **state) {
	(void)state;
	fake_memory_fail_on_all_call();
	stream *ret = stdio_stream_to_stdout();
	fake_memory_fail_on_calls(0, NULL);
	assert_null(ret);
}
...
static void to_stdout_write_to_stdout(void **state) {
	...
	stream *s = stdio_stream_from_fp_for_tests(fake_stdout, true);
	assert_non_null(s);
	size_t n = stream_write(s, buf, 5);
