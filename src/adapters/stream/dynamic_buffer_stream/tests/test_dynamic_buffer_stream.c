// src/adapters/stream/dynamic_buffer_stream/tests/test_dynamic_buffer_stream.c
...
static void create_ret_null_when_oom(void **state) {
	...
	stream *ret = dynamic_buffer_stream_create();
...
static void create_initialize_ctx_when_no_oom(void **state) {
	...
	assert_non_null(ret);
	dynamic_buffer_stream_ctx *ctx = (dynamic_buffer_stream_ctx*)stream_get_ctx(ret);
	assert_non_null(ctx);
	...
	assert_int_equal(stream_close(ret), 0);
}
...
static void write_success_when_default_cap_is_sufficient(void **state) {
	...
	// ARRANGE
	stream *dbs = dynamic_buffer_stream_create();
	assert_non_null(dbs);
	dynamic_buffer_stream_ctx *ctx = (dynamic_buffer_stream_ctx*)stream_get_ctx(dbs);
	assert_non_null(ctx);
	assert_non_null(ctx->buf);
	char *str_to_be_written = "I will be written in a dynamic buffer.";

	// ACT
	size_t ret = stream_write(dbs, str_to_be_written, strlen(str_to_be_written));
	...
	assert_int_equal(stream_close(dbs), 0);
}
...
static void write_success_when_default_cap_is_not_sufficient(void **state) {
	...
	// ACT
	size_t ret = stream_write(dbs, str_to_be_written, len);
...
static void read_ret_0_and_no_side_effect_when_buf_empty(void **state) {
	...
	// ACT
	size_t ret = stream_read(dbs, &read_char, 1);
...
static void read_read_a_string_when_it_is_written_in_buf(void **state) {
	...
	assert_int_equal(stream_write(dbs, str_to_be_written, n), n);
	assert_int_equal(ctx->len, n);
	assert_int_equal(ctx->read_pos, 0);
	char read_buf[DYNAMIC_BUFFER_STREAM_DEFAULT_CAPACITY] = {0};

	// ACT
	size_t ret = stream_read(dbs, read_buf, n);
...
static void read_try_to_1000_bytes_when_10_are_already_read(void **state) {
	...
	size_t n = strlen(str_to_be_written) + 1;
	assert_int_equal(stream_write(dbs, str_to_be_written, n), n);
	assert_int_equal(stream_read(dbs, read_buf, 10), 10);
	assert_int_equal(ctx->len, n);
	assert_int_equal(ctx->read_pos, 10);

	// ACT
	size_t ret = stream_read(dbs, read_buf, 1000);
...
static void read_try_to_1000_bytes_when_all_is_already_read(void **state) {
	...
	size_t n = strlen(str_to_be_written) + 1;
	assert_int_equal(stream_write(dbs, str_to_be_written, n), n);
	assert_int_equal(stream_read(dbs, read_buf, n), n);
	assert_int_equal(ctx->len, n);
	assert_int_equal(ctx->read_pos, n);
