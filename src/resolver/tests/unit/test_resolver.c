// src/resolver/tests/unit/test_resolver.c
...
static int resolve_ast_setup(void **state) {
    ...
    ctx.ops.push = list_push; // real
    ctx.ops.pop = list_pop; // real
    ctx.ops.intern_symbol = mock_intern_symbol; // mocked
    ctx.ops.get = mock_get; // mocked
    ctx.st = &STUB_SYMTAB_INSTANCE; // stubbed
    fake_memory_reset();
    return 0;
}
...
// intern_symbol is not called because symbol name data
// wrapper child is turned into error sentinel node before
// its parsing

// only one more with happy path
