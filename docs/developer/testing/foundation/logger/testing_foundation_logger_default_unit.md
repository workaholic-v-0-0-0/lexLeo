...
| WHEN | EXPECT |
|---|---|
| `logger_default_default_env(stream, adapter_mem, port_env)` is called with valid inputs | returns a `logger_default_env_t` such that `env.stream == stream`, `env.adapter_mem == adapter_mem`, and `env.port_env == *port_env` |

## Notes

- This helper preserves the injected stream and logger-port dependencies provided by the caller.
- The returned environment does not take ownership of the borrowed `stream` handle or borrowed `adapter_mem` table.