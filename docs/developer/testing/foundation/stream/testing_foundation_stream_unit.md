@page testing_foundation_stream_unit stream unit tests

@anchor testing_foundation_stream_unit_stream_default_ops
# stream_default_ops() unit tests

See @ref specifications_stream_default_ops "stream_default_ops() specifications"
...
| WHEN | EXPECT                                                                                      |
|---|---------------------------------------------------------------------------------------------|
| `stream_default_ops()` is called | ret` is non-`NULL;<br>`ret->read != NULL`;<br>`ret->write != NULL`;<br>`ret->flush != NULL` |
...
@anchor testing_foundation_stream_unit_stream_lifecycle
# stream lifecycle unit tests

See @ref specifications_stream_create "stream_create() specifications"
See @ref specifications_stream_complete_default_init "stream_complete_default_init() specifications"
See @ref specifications_stream_destroy "stream_destroy() specifications"
...
| Scenario | WHEN | EXPECT |
|---|---|---|
| Nominal lifecycle | The following lifecycle is executed:<br>`stream_create(out, env)`;<br>`stream_complete_default_init(*out, backend)`;<br>`stream_destroy(out)` | `stream_create()` returns `STREAM_STATUS_OK`;<br>`*out != NULL`;<br>the newly created stream has no backend;<br>`stream_complete_default_init()` returns `STREAM_STATUS_OK`;<br>the stream backend is set to `backend`;<br>`stream_destroy()` returns `STREAM_STATUS_OK`;<br>releases the backend and the stream handle;<br>sets `*out` to `NULL`;<br>no memory leak, invalid free or double free occurs |
| Stream allocation failure | Allocation of the stream handle fails during `stream_create(out, env)` | `stream_create()` returns `STREAM_STATUS_OOM`;<br>`*out == NULL`;<br>no memory leak, invalid free or double free occurs |
| Idempotent destruction | After a successfully completed stream initialization, `stream_destroy(out)` is called twice | the first call returns `STREAM_STATUS_OK`;<br>releases the backend and the stream handle;<br>sets `*out` to `NULL`;<br>invokes the backend close operation exactly once;<br>the second call returns `STREAM_STATUS_OK`;<br>leaves `*out == NULL`;<br>does not invoke the backend close operation again;<br>no memory leak, invalid free or double free occurs |
...
See @ref specifications_stream_write stream_write() specifications
