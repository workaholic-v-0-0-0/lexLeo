...
This page describes the integration tests validating
...
- Unless stated otherwise by the scenario, `args` designates a valid
  `stream_file_creator_args_t` object prepared by the fixture.
...
- `args->mode` must not be `NULL` and must designate a supported file-opening
  mode.
...
| `args != NULL` but `args->mode == NULL` | returns `STREAM_STATUS_INVALID`;<br>leaves `*out` unchanged |
| `args != NULL` but `args->mode` does not designate a supported file-opening mode | returns `STREAM_STATUS_INVALID`;<br>leaves `*out` unchanged |
...
- The `fs_stream` adapter descriptor is created through
  `fs_stream_create_desc()` and registered into the factory before the call
  under test.
- Backend file-open failures are injected through `fake_file`.
- In the nominal success scenario, the produced stream is additionally
  validated through the public borrower API:
    - `stream_write()`
    - `stream_flush()`
