@page specifications_fs_stream_create_adapter_provider fs_stream_create_adapter_provider() specifications
...
Create a `stream_adapter_provider_t` for the `fs_stream` adapter, suitable for
registration in a `stream_factory_t` through `stream_factory_add_adapter()`.

The produced provider supplies everything required by the stream factory to
construct `fs_stream` backends and operate them through the `stream` port.
...
- Returns `FS_STREAM_STATUS_OOM` if a required allocation fails.
...
- This function provides the `fs_stream`-specific adapter provider required by
  the generic stream factory.
- `backend_ctor` defines how `fs_stream` backends are constructed.
