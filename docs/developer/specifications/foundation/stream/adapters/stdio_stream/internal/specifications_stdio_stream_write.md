...
- Returns the number of bytes reported by the underlying OSAL standard I/O
  write operation.
- The underlying standard stream does not report an error.
...
- After invoking the injected OSAL standard I/O write operation, if the
    underlying standard stream reports an error, returns
    `STREAM_STATUS_IO_ERROR` through `st`, when `st != NULL`.
- An underlying standard I/O error does not imply that no bytes were written:
  the function returns the number of bytes reported by the underlying write
  operation.
