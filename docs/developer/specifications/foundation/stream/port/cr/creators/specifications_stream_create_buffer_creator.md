@page specifications_stream_create_buffer_creator stream_create_buffer_creator() specifications
...
Create a buffer stream creator bound to a `stream_factory_t` and adapter key.
...
- Stores a valid newly created `stream_buffer_creator_t` handle in `*out`.
...
- On success, ownership of the produced `stream_buffer_creator_t` handle is
