@page specifications_stream_create_regular_file_creator stream_create_regular_file_creator() specifications
...
Create a creator for regular file streams.
...
- Stores a valid newly created `stream_regular_file_creator_t` handle in `*out`.
- The returned creator can subsequently be passed to
  `stream_file_creator_create()` to create regular file-backed `stream_t`
  handles.
...
- On success, ownership of the created `stream_regular_file_creator_t` handle is
