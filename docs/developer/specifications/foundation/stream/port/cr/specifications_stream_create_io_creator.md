@page specifications_stream_create_io_creator stream_create_io_creator() specifications
...
Create an I/O stream creator bound to a `stream_factory_t` and adapter key.
...
- Stores a valid newly created `stream_io_creator_t` handle in `*out`.
...
- On success, ownership of the produced `stream_io_creator_t` handle is
